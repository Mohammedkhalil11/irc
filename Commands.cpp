...
void Server::commands(std::string msg,std::vector<struct pollfd> fds, int index)
{
    ...
    else if(str == "PRIVMSG")
    {
        ...
        if(ss.fail())
        {
            ...
            // ERR_NORECIPIENT format
            response << ":WEBSERV 411 " << Clients[fds[index].fd].nickNameGetter() << " :No recipient given (PRIVMSG)\r\n";
        ...
        if(str[0] == '#')
        {
            ...
            {
                std::ostringstream response;
                response << ":WEBSERV 401 " << Clients[fds[index].fd].nickNameGetter() << " " << str << " :No such nick/channel\r\n";
                // Send the response to the client
                send(fds[index].fd, response.str().c_str(), response.str().size(), 0);
                return;
            }
            else
            {
                ...
                if (ss.fail())
                {
                    std::ostringstream privError;
                    privError<<":WEBSERV 461 "<<Clients[fds[index].fd].nickNameGetter()<<" PRIVMSG :Not enough parameters\r\n";
                    send(fds[index].fd, privError.str().c_str(), privError.str().size(), 0);
                    return;
                }
                std::string msg;
                std::string line;
